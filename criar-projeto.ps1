...
#  Uso (no PowerShell):   powershell -ExecutionPolicy Bypass -File D:\03-PESSOAL\_BASE-APP\criar-projeto.ps1
# ============================================================================
...
# 1. Nome
$nome = Read-Host 'Nome do app (ex.: Controle de Estoque)'
if (-not $nome.Trim()) { throw 'Informe um nome.' }
$sugestaoPasta = ($nome.Trim().ToUpper() -replace '[^A-Z0-9]+', '-').Trim('-')
$pasta = Read-Host "Nome da pasta [$sugestaoPasta]"
if (-not $pasta.Trim()) { $pasta = $sugestaoPasta }
$nomeCurto = Read-Host "Nome curto, embaixo do icone (ate 12 letras) [$($nome.Trim().Split(' ')[0])]"
...
$mapa = @{ '1' = 'tablet'; '2' = 'celular'; '3' = 'pc' }
$dispositivos = @($escolha -split '[,; ]+' | Where-Object { $mapa.ContainsKey($_) } | ForEach-Object { $mapa[$_] } | Select-Object -Unique)
if ($dispositivos.Count -eq 0) { throw 'Escolha pelo menos um dispositivo (1, 2 ou 3).' }

# 3. Cor
$cor = Read-Host 'Cor principal em hexadecimal [#2f6fed]'
if (-not ($cor -match '^#[0-9a-fA-F]{6}$')) { $cor = '#2f6fed' }

# 4. Onde criar
$pai = Split-Path $base -Parent
...
$t = Ler-Texto $cfg
$lista = ($dispositivos | ForEach-Object { "'$_'" }) -join ', '
...
$readme = "# $($nome.Trim())`n`nCriado a partir da base (_BASE-APP). Dispositivos: $($dispositivos -join ', ').`n`nVeja COMO-USAR.md, DESIGN.md e ARQUITETURA.md.`n"
...
git add -A | Out-Null
git commit -q -m "Projeto criado a partir da base ($($dispositivos -join ', '))" | Out-Null
Pop-Location

Write-Host ''
Write-Host "Pronto! Projeto criado em $destino" -ForegroundColor Green
Write-Host "Dispositivos: $($dispositivos -join ', ')"
Write-Host ''
